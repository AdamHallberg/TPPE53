function [A, B, alpha_1, gamma_end] = crank_nicholson_coeff(S_j, r, sigma, dt, dS)
    ...
    a = (sigma^2 * S_j.^2) / (2 * dS^2) - (r * S_j) / (2 * dS);
    b = - (sigma^2 * S_j.^2) / (dS^2) - r;
    c = (sigma^2 * S_j.^2) / (2 * dS^2) + (r * S_j) / (2 * dS);
    ...
    alpha_1 = (sigma^2 * S_j(1)^2) / (2 * dS^2) - (r * S_j(1)) / (2 * dS);
    gamma_end = (sigma^2 * S_j(end)^2) / (2 * dS^2) + (r * S_j(end)) / (2 * dS);
